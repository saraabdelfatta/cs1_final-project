#include <iostream> 
#include <fstream>
#include <stdlib.h> //what is meant by this libarry ? where we uesed it?
using namespace std;
...
{
public:
    customer user[50];  // user name - id_number - passward - adress -email//
    char answer;    //what is for?
    int x;  //------------------------------------------for what?
    char ans;   //the same what is for?
    bool y;
    accounts()       //why you call the class again?
    {
        x = 0;
        ans = 'n';
        y=0;
...
class product
{
    ...
    product()   // why tou call the product?
    {
        x=0;
        category ="";
    }
...
public:
    ...
    product prod;   //inctance product 
    cart incart[50];            // inctaNCE CART NAME PRICE AMMOUNT 
    payment pay;
	int sh;
	char ch;
    int neg;
